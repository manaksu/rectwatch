...
function loadCfg(){return{he:+(localStorage.getItem('he')||'1'),st:+(localStorage.getItem('st')||'1'),th:+(localStorage.getItem('th')||'0'),bo:+(localStorage.getItem('bo')||'0'),ep:+(localStorage.getItem('ep')||'1'),co:+(localStorage.getItem('co')||'0'),br:+(localStorage.getItem('br')||'1')};}
function saveCfg(c){localStorage.setItem('he',c.he);localStorage.setItem('st',c.st);localStorage.setItem('th',c.th);localStorage.setItem('bo',c.bo);localStorage.setItem('ep',c.ep);localStorage.setItem('co',c.co);localStorage.setItem('br',c.br);}
function sendMsg(c){
  var keys=[[0,c.he],[1,c.st],[2,c.th],[3,c.bo],[4,c.ep],[5,c.co],[6,c.br]];
...
function buildConfig(c){
  ...
  var script="document.getElementById('s').onclick=function(){function g(n){var e=document.querySelector('input[name='+n+']:checked');return e?+e.value:0;}location.href='pebblejs://close#'+encodeURIComponent(JSON.stringify({he:g('he'),st:g('st'),th:g('th'),bo:g('bo'),ep:g('ep'),co:g('co'),br:g('br')}));};";
  var html='<!-- v10 --><!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>'+style+'</style></head><body>'
    ...
    +'<h3>Clock Border</h3>'+radio('br',['Off','On'],c.br)
