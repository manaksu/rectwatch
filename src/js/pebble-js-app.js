/* RectWatch JS — A_HEALTH=0, B_STEPS=1, C_THEME=2 */
function loadCfg(){return{he:+(localStorage.getItem('he')||'1'),st:+(localStorage.getItem('st')||'1'),th:+(localStorage.getItem('th')||'0')};}
function saveCfg(c){localStorage.setItem('he',c.he);localStorage.setItem('st',c.st);localStorage.setItem('th',c.th);}
function sendMsg(c){
  var keys=[[0,c.he],[1,c.st],[2,c.th]];
...
function buildConfig(c){
  ...
  var script="document.getElementById('s').onclick=function(){function g(n){var e=document.querySelector('input[name='+n+']:checked');return e?+e.value:0;}location.href='pebblejs://close#'+encodeURIComponent(JSON.stringify({he:g('he'),st:g('st'),th:g('th')}));};";
  var html='<!-- v5 --><!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>'+style+'</style></head><body>'
