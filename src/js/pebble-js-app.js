 ...
 *   A_HEALTH -> 0   0=hide  1=show
 *   B_STEPS  -> 1   0=hide  1=show
 *   C_THEME  -> 2   0=light 1=dark
 */
function loadCfg() {
  return {
    he: +(localStorage.getItem('he') || '1'),
    st: +(localStorage.getItem('st') || '1'),
    th: +(localStorage.getItem('th') || '0')
  };
}
function saveCfg(c) {
  localStorage.setItem('he', c.he);
...
function sendMsg(c) {
  ...
  function next(i) {
    ...
    Pebble.sendAppMessage(msg,function(){next(i+1);},function(){next(i+1);});
  }
  next(0);
}
function radio(name,opts,sel){
  return opts.map(function(l,i){
    var ch=(i===sel)?' checked':'';
    return '<label class="opt"><input type="radio" name="'+name+'" value="'+i+'"'+ch+'><span>'+l+'</span></label>';
  }).join('');
}
function buildConfig(c) {
  ...
  var script="document.getElementById('s').onclick=function(){"
    ...
    +"location.href='pebblejs://close#'+encodeURIComponent(JSON.stringify({he:g('he'),st:g('st'),th:g('th')}));"
    +"};";
  var html='<!-- v4 --><!DOCTYPE html><html><head>'
