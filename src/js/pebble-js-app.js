...
function loadCfg(){return{he:+(localStorage.getItem('he')||'1'),st:+(localStorage.getItem('st')||'1'),th:+(localStorage.getItem('th')||'0'),bo:+(localStorage.getItem('bo')||'0'),ep:+(localStorage.getItem('ep')||'1')};}
function saveCfg(c){localStorage.setItem('he',c.he);localStorage.setItem('st',c.st);localStorage.setItem('th',c.th);localStorage.setItem('bo',c.bo);localStorage.setItem('ep',c.ep);}
function sendMsg(c){
  var keys=[[0,c.he],[1,c.st],[2,c.th],[3,c.bo],[4,c.ep]];
...
function buildConfig(c){
  ...
  var script="document.getElementById('s').onclick=function(){function g(n){var e=document.querySelector('input[name='+n+']:checked');return e?+e.value:0;}location.href='pebblejs://close#'+encodeURIComponent(JSON.stringify({he:g('he'),st:g('st'),th:g('th'),bo:g('bo'),ep:g('ep')}));};";
  var html='<!-- v7 --><!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>'+style+'</style></head><body>'
